...
import { Minus, Plus, ShoppingBag, ShoppingCart, RefreshCw } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto">
      <header className="mb-6">
        ...
        <p className="text-sm text-center" style={{ color: COLORS.textSecondary }}>
          Scan QR code or visit the link to order
        </p>
      </header>

      {totalItems > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-lg shadow-sm sticky mb-6 transition-all duration-200"
          style={{
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-sm mt-1" style={{ color: COLORS.textSecondary }}>
            Ask administrator to add items in Management
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      {totalItems > 0 && (
        <div className="mt-8">
          <button
            onClick={() => {
              // Navigate to checkout flow for public menu
