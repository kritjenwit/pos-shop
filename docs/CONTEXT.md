...
### Thai QR Payment Branding
- **Scan to Pay layout**: Three-layer vertical stack — Thai QR Payment logo (Logo-01.jpg), then PromptPay badge (PromptPay2.png), then QR code with Thai QR Payment badge (Logo-06.png) centered as overlay
- **Assets**: Stored in Supabase `pos-shop` bucket, fetched via `getSignedImageUrl()` (same pattern as item images)
